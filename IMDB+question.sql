...
-- So columns which have null values are country,worlwide_gross_income,languages,production_company
...
/* We see a decrease in no. of movies produced per year. No Peculiar Trend observed monthly like increasing or decreasing. Highest number of movies produced 
in month of March and lowest in month of December. */
...
-- Drama,Comedy genre had the highest number of movies produced overall; whereas Fantasy,Family and Others are lowest.
...
GROUP BY movie_id
HAVING Count(genre)=1) as Count_Table;

-- 3289 Movies have only 1 Genre
...
-- Action Genre had Highest Average Duration at around 112 Minutes while Horror had Lowest at around 92 Minutes
...
-- Thriller Movies are Ranked at 3
...
SELECT
	country,
    sum(total_votes) as movie_count
FROM
movie m
INNER JOIN
ratings r
...
-- No Nulls found in name column.Nulls found in height,date_of_birth and known_for_movies columns.
...
top_director AS (
    ...
    INNER JOIN
        genre g ON g.movie_id = d.movie_id
    INNER JOIN
        top_genre tg ON tg.genre = g.genre
    WHERE
        r.avg_rating > 8
    GROUP BY
        n.name
...
-- Andrew Jones and A.L. Vijay is most popular in  term of number of movies directed


-- Extra Code for some further analysis done
...
-- The above code gives directors who show highest average rating and directed atleast 2 movies. James Mangold wins here.
...
INNER JOIN genre g
ON g.movie_id = r.movie_id
WHERE category="actor" AND avg_rating>8 AND g.genre="Drama"
GROUP BY name
LIMIT 3;

-- Actor Andrew Garfield Shows Highest potential in drama genre
...
-- Highly Successful drama movies should have duration around 117 mins
